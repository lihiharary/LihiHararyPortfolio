...
// This class tells CSS that JS is working, so CSS can safely hide elements for animation
...
window.openProject = function(projectId) {
    ...
    if (!container || !content) {
        console.error("Modal container or content not found for:", projectId);
        return;
    }
...
/* ==========================================================================
   4. Scroll Animations (Bulletproof Version)
   ========================================================================== */

document.addEventListener("DOMContentLoaded", function() {
    const observerOptions = {
        root: null,
        threshold: 0.05, // ירדנו ל-5% במקום 15%, שיופעל מיד כשזה נכנס למסך
        rootMargin: "0px"
    };
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            ...
            // הסרנו את ההיעלמות חזרה, כדי למנוע היעלמות בטעות כשגוללים מעלה ומטה!
        });
    }, observerOptions);

    const sectionsToReveal = document.querySelectorAll(
        '.reveal-section, .reveal-grid, .stagger-item, .fade-up, .big-contact-header, .slide-from-left, .slide-from-right'
    );
...
const translations = {
    ...
    he: {
        ...
        contact_big_header: "Contact", contact_btn_work: "בואו נעבוד יחד", contact_btn_hi: "סתם להגיד היי",
